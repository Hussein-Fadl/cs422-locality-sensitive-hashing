...
class LSHIndex(data: RDD[(Int, String, List[String])], seed : IndexedSeq[Int]) extends Serializable {
  ...
  def getBuckets()
    ...
    val t1 : RDD[(List[String], IndexedSeq[Int])] = hash(data.map(x=>x._3)).map(x=>(x._2,x._1))
    val t2 : RDD[(List[String], (Int, String, List[String]))]= data.map(x=>(x._3,x))

    val buckets =  t1.join(t2).map(x=>(x._2._1, x._2._2)).distinct()

    buckets.groupByKey().mapValues(_.toList)
  }
  ...
  : RDD[(IndexedSeq[Int], T, List[(Int, String, List[String])])] = {
    ...
    val joined = queries.leftOuterJoin(getBuckets())
    joined.map{ case (signature,(query, movies)) => (signature, query, movies.getOrElse(List.empty))}
